...
string read(string filepath) {
    ...
    contentFile.seekg(0, std::ios::end);
    size_t size = contentFile.tellg();
    string buffer(size, ' ');
    contentFile.seekg(0);
    contentFile.read(&buffer[0], size);
    content = buffer;
    contentFile.close();
...
int main(int argc, char** argv) {
    ...
    string curver = read(rootDir + "\\programversion.rco");
    if (curver != programver) {
        STARTUPINFOA si;
