...
int main(int argc, char* argv[]) {
    ...
    std::string updDir("C:\\RCO2Updater");
    ...
    if (userIn == std::string("install")) { // Install
        ...
        if (std::filesystem::exists(rootDir + "\\libcurl.dll") == false) {
            ...
            CURL* req = curl_easy_init();
            CURLcode res;
            curl_easy_setopt(req, CURLOPT_URL, "https://raw.githubusercontent.com/fheahdythdr/rco-but-it-uses-different-fflags/main/libcurl.dll");
            curl_easy_setopt(req, CURLOPT_HTTP_VERSION, CURL_HTTP_VERSION_2TLS);
            curl_easy_setopt(req, CURLOPT_SSLVERSION, CURL_SSLVERSION_TLSv1_2);
            curl_easy_setopt(req, CURLOPT_WRITEFUNCTION, NULL);
            curl_easy_setopt(req, CURLOPT_WRITEDATA, file);
        ...
        if (std::filesystem::exists(updDir + "\\libcurl.dll") == false) {
            ...
            CURL* req = curl_easy_init();
            CURLcode res;
            curl_easy_setopt(req, CURLOPT_URL, "https://raw.githubusercontent.com/fheahdythdr/rco-but-it-uses-different-fflags/main/RCO2Updater/libcurl.dll");
            curl_easy_setopt(req, CURLOPT_HTTP_VERSION, CURL_HTTP_VERSION_2TLS);
            curl_easy_setopt(req, CURLOPT_SSLVERSION, CURL_SSLVERSION_TLSv1_2);
            curl_easy_setopt(req, CURLOPT_WRITEFUNCTION, NULL);
            curl_easy_setopt(req, CURLOPT_WRITEDATA, file);
        ...
        if (std::filesystem::exists(updDir + "\\zlib1.dll") == false) {
            ...
            CURL* req = curl_easy_init();
            CURLcode res;
            curl_easy_setopt(req, CURLOPT_URL, "https://raw.githubusercontent.com/fheahdythdr/rco-but-it-uses-different-fflags/main/RCO2Updater/zlib1.dll");
            curl_easy_setopt(req, CURLOPT_HTTP_VERSION, CURL_HTTP_VERSION_2TLS);
            curl_easy_setopt(req, CURLOPT_SSLVERSION, CURL_SSLVERSION_TLSv1_2);
            curl_easy_setopt(req, CURLOPT_WRITEFUNCTION, NULL);
            curl_easy_setopt(req, CURLOPT_WRITEDATA, file);
        ...
        if (std::filesystem::exists(updDir + "\\RCO-Updater.exe") == false) {
            FILE* file;
            if (fopen_s(&file, (updDir + "\\RCO-Updater.exe").c_str(), "wb") != 0) {
                std::cout << "Could not open filepoint... | 0x3\n";
                std::cin.get();
                return 1;
            }
            CURL* req = curl_easy_init();
            CURLcode res;
            curl_easy_setopt(req, CURLOPT_URL, "https://raw.githubusercontent.com/fheahdythdr/rco-but-it-uses-different-fflags/main/RCO2Updater.exe");
            curl_easy_setopt(req, CURLOPT_HTTP_VERSION, CURL_HTTP_VERSION_2TLS);
            curl_easy_setopt(req, CURLOPT_SSLVERSION, CURL_SSLVERSION_TLSv1_2);
            curl_easy_setopt(req, CURLOPT_WRITEFUNCTION, NULL);
            curl_easy_setopt(req, CURLOPT_WRITEDATA, file);
        ...
        if (std::filesystem::exists(rootDir + "\\zlib1.dll") == false) {
            ...
            CURL* req = curl_easy_init();
            CURLcode res;
            curl_easy_setopt(req, CURLOPT_URL, "https://raw.githubusercontent.com/fheahdythdr/rco-but-it-uses-different-fflags/main/zlib1.dll");
            curl_easy_setopt(req, CURLOPT_HTTP_VERSION, CURL_HTTP_VERSION_2TLS);
            curl_easy_setopt(req, CURLOPT_SSLVERSION, CURL_SSLVERSION_TLSv1_2);
            curl_easy_setopt(req, CURLOPT_WRITEFUNCTION, NULL);
            curl_easy_setopt(req, CURLOPT_WRITEDATA, file);
